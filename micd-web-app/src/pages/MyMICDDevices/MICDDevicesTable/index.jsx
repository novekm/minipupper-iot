...
// import { deleteMICDDevice } from '../../../graphql/mutations';

import { getFilterCounterText } from '../../../common/resources/tableCounterStrings';
import { FullPageHeader, MyMICDDevicesTableEmptyState } from '../index';
import {
  CustomAppLayout,
  Navigation,
  TableNoMatchState,
...
const MICDDevicesTable = ({ updateTools, saveWidths, columnDefinitions }) => {
  ...
  const [micdDevices, setMicdDevices] = useState([]);
  // const [selectedTranscripts, setSelectedTranscripts] = useState([]);

  const [distributions, setDistributions] = useState([]);
  const [preferences, setPreferences] = useLocalStorage(
    'React-MICDDevices-Table-Preferences',
    DEFAULT_PREFERENCES
  );
  ...
  const {
    ...
    paginationProps,
    propertyFilterProps,
  } = useCollection(micdDevices, {
    propertyFiltering: {
      filteringProperties: FILTERING_PROPERTIES,
      empty: <MyMICDDevicesTableEmptyState resourceName="MICD Devices" />,
      noMatch: (
        <TableNoMatchState
          onClearFilter={() => {
  ...
  const fetchMICDDevices = async () => {
    try {
      const MICDDeviceData = await API.graphql(
        graphqlOperation(listIoTDevices, { limit: 10000 })
      );
      const MICDDeviceDataList = MICDDeviceData.data.listIoTDevices.devices;
      console.log('MICD Devices List', MICDDeviceDataList);
      setMicdDevices(MICDDeviceDataList);
      setLoading(false);
    } catch (error) {
      console.log('error on fetching MICD Devices', error);
    }
  };
    ...
    <Table
      ...
      header={
        <FullPageHeader
          ...
          totalItems={micdDevices}
          updateTools={updateTools}
          serverSide={false}
        />
      }
      loading={loading}
      loadingText="Loading MICD Devices..."
      filter={
        <PropertyFilter
...
export default MICDDevicesTable;
